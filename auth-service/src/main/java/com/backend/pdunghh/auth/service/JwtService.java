package com.backend.pdunghh.auth.service;
...
import com.backend.pdunghh.auth.config.AuthProperties;
import com.backend.pdunghh.auth.entity.RefreshTokenEntity;
import com.backend.pdunghh.auth.entity.UserEntity;
import com.backend.pdunghh.auth.repository.RefreshTokenRepository;
import com.backend.pdunghh.shared.util.HashUtils;
...
public class JwtService {
     ...
     * <p>
     * Khóa bí mật được tự động tạo ra từ chuỗi {@code jwtSecret} trong cấu hình ứng
     * dụng,
     * sử dụng thuật toán HMAC-SHA để đảm bảo an toàn tuyệt đối cho việc ký token.
     *
     * @param authProperties         Cấu hình chứa khóa bí mật và thời hạn sinh
     *                               token.
     * @param refreshTokenRepository Repository để tương tác với bảng lưu trữ
     *                               Refresh Token.
     */
     ...
     * Thời gian sống của token được thiết lập ngắn (nhằm giảm thiểu rủi ro bảo mật
     * nếu lộ token),
     * tuân thủ nghiêm ngặt theo cấu hình {@code accessTokenMinutes} trong hệ thống.
     *
     * @param user Đối tượng chứa thông tin người dùng cần cấp phát token, không
     *             được {@code null}.
     * @return Chuỗi JWT (Access Token) đã được ký điện tử an toàn, sẵn sàng trả về
     *         Client.
     */
     ...
     * <p>
     * Phương thức này kiểm tra xem chữ ký của token có khớp với khóa bí mật hay
     * không (chống giả mạo),
     * và token đã hết hạn hay chưa. Nếu token không hợp lệ, thư viện JJWT sẽ tự
     * động
     * ném ra các ngoại lệ tương ứng.
     *
     * @param token Chuỗi Access Token cần giải mã (thường lấy từ Header
     *              Authorization).
     * @return Đối tượng {@link Claims} chứa toàn bộ thông tin (payload) bên trong
     *         token.
     * @throws io.jsonwebtoken.JwtException Nếu token bị sai chữ ký, đã hết hạn,
     *                                      hoặc có định dạng không hợp lệ.
     */
     ...
     * <p>
     * Token được sinh ngẫu nhiên dạng UUID. Để phòng chống rủi ro lộ lọt dữ liệu
     * (Database Leak),
     * chuỗi token sẽ được băm bằng thuật toán SHA-256 thông qua {@link HashUtils}
     * trước khi lưu.
     * Thời hạn của Token tự động lấy từ cấu hình {@code refreshTokenDays}.
     *
     * @param user Thực thể người dùng sở hữu Refresh Token này, không được
     *             {@code null}.
     * @return Chuỗi Refresh Token gốc dạng ký tự thuần (chưa băm) để gửi trả về cho
     *         Client.
     */
